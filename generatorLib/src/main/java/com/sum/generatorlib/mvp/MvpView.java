...
public class MvpView {
    ...
    public void generator(String temp) throws Exception {
        ...
        //拼接类引入包路径
        String packageImport = ToolUtils.getImportPackage(Generator.BASE_PACKAGE, Generator.PATH_APP, Generator.PATH, Generator.VIEW_PATH);
        root.put("packageImport", packageImport);
        root.put("viewName", Generator.VIEW_NAME);
        root.put("desc", Generator.DESC);
        //文件生成路径
        String filePath = ToolUtils.getFilePath(Generator.BASE_PACKAGE, Generator.PATH_APP, Generator.PATH, Generator.VIEW_PATH);
        File file = new File(filePath, Generator.VIEW_NAME + ".java");
